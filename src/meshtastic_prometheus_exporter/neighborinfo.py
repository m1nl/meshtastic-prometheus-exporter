...
def on_meshtastic_neighborinfo_app(redis, packet, source_long_name, source_short_name):
    ...
    for n in neighbor_info["neighbors"]:
        ...
        neighbor_info_attributes["neighbor_source_long_name"] = (
            get_decoded_node_metadata_from_redis(redis, neighbor_source, "long_name") if source else "unknown"
        )
        neighbor_info_attributes["neighbor_source_short_name"] = (
            get_decoded_node_metadata_from_redis(redis, neighbor_source, "short_name") if source else "unknown"
        )

        if "snr" in n:
            meshtastic_neighbor_info_snr_decibels.set(n["snr"], attributes=neighbor_info_attributes)
