...
def on_meshtastic_telemetry_app(redis, packet, attributes):
    ...
    if "powerMetrics" in telemetry:
        ...
        if "ch1_voltage" in powerMetrics:
            meshtastic_telemetry_power_ch1_voltage_volts.set(
                powerMetrics["ch1_voltage"],
                attributes=telemetry_attributes,
            )
        if "ch1_current" in powerMetrics:
            meshtastic_telemetry_power_ch1_current_amperes.set(
                powerMetrics["ch1_current"] * 10**-3,
                attributes=telemetry_attributes,
            )
        if "ch2_voltage" in powerMetrics:
            meshtastic_telemetry_power_ch2_voltage_volts.set(
                powerMetrics["ch2_voltage"],
                attributes=telemetry_attributes,
            )
        if "ch2_current" in powerMetrics:
            meshtastic_telemetry_power_ch2_current_amperes.set(
                powerMetrics["ch2_current"] * 10**-3,
                attributes=telemetry_attributes,
            )
        if "ch3_voltage" in powerMetrics:
            meshtastic_telemetry_power_ch3_voltage_volts.set(
                powerMetrics["ch3_voltage"],
                attributes=telemetry_attributes,
            )
        if "ch3_current" in powerMetrics:
            meshtastic_telemetry_power_ch3_current_amperes.set(
                powerMetrics["ch3_current"] * 10**-3,
                attributes=telemetry_attributes,
            )
